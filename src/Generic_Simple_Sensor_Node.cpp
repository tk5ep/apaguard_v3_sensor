...
#define BME_TEMP_SENSOR
#define BME_HUM_SENSOR
#define BME_PRESS_SENSOR
...
unsigned int idlePeriodInMin = 15;
...
// SUMMARY OF LPP CHANNELS USED BY THE DEVICE
    ...
    // ch12: weight ADDED TK5EP
    // ch13: pression added tk5ep
    //...
...
// LORAWAN OR EXTENDED DEVICE ADDRESS FOR LORAWAN CLOUD
    ...
    unsigned char DevAddr[4] = {0x26, 0x01, 0x1D, 0xAB};
#endif
#else
    ///////////////////////////////////////////////////////////////////
    // DO NOT CHANGE HERE
...
#ifdef BME_TEMP_SENSOR
    #include "BME_Temperature.h"
#endif

#ifdef BME_HUM_SENSOR
    #include "BME_Humidity.h"
#endif

#ifdef BME_PRESS_SENSOR
    #include "BME_Pressure.h"
#endif
...
// AJOUT TK5EP
    ...
    #include <EEPROM.h>
    #define EEPROM_ADDR_OFFSET 20 // où sont stockées les valeurs de l'offset et scale après calibration par script externe
    #define EEPROM_ADDR_SCALE  24
#endif

#ifdef DS3231_ENABLED
    #include "RTC.h"
#endif
...
const float VccCorrection = 1.0; //3.0 / 2.9;
...
// ST
...
#ifdef BME_TEMP_SENSOR
    sensor_ptrs[sensor_index] = new BME_Temperature((char*)"AT", IS_NOT_ANALOG, IS_CONNECTED, low_power_status, 
                                                    (uint8_t)BME_SDA_PIN, (uint8_t)BME_PWR_PIN, (uint8_t)BME_SCL_PIN);
    sensor_ptrs[sensor_index]->set_n_sample(1);
    sensor_ptrs[sensor_index]->set_warmup_time(100); // Seul celui-ci chauffe
    sensor_index++;
#endif

#ifdef BME_HUM_SENSOR
    sensor_ptrs[sensor_index] = new BME_Humidity((char*)"AH", IS_NOT_ANALOG, IS_CONNECTED, low_power_status, 
                                                 (uint8_t)BME_SDA_PIN, (uint8_t)BME_PWR_PIN, (uint8_t)BME_SCL_PIN);
    sensor_ptrs[sensor_index]->set_n_sample(1);
    sensor_ptrs[sensor_index]->set_warmup_time(0);
    sensor_ptrs[sensor_index]->set_is_power_on_when_active(false); // Déjà allumé par bme_temp
    sensor_index++;
#endif

#ifdef BME_PRESS_SENSOR
    sensor_ptrs[sensor_index] = new BME_Pressure((char*)"AP", IS_NOT_ANALOG, IS_CONNECTED, low_power_status, 
                                                 (uint8_t)BME_SDA_PIN, (uint8_t)BME_PWR_PIN, (uint8_t)BME_SCL_PIN);
    sensor_ptrs[sensor_index]->set_n_sample(1);
    sensor_ptrs[sensor_index]->set_warmup_time(0);
    sensor_ptrs[sensor_index]->set_is_power_on_when_active(false); // Déjà allumé par bme_temp
    ...
    // lecture des données dans EEPROM
    long saved_offset;
    float saved_scale;
    EEPROM.get(EEPROM_ADDR_OFFSET, saved_offset);
    EEPROM.get(EEPROM_ADDR_SCALE, saved_scale);

    //test
    PRINTLN_CSTSTR("EEPROM lue");
    Serial.println(saved_offset);
    Serial.println(saved_scale,6);
    //

    sensor_ptrs[sensor_index] = new BME_Humidity((char*)"AH", IS_NOT_ANALOG, IS_CONNECTED, low_power_status, 
                                                 (uint8_t)BME_SDA_PIN, (uint8_t)BME_PWR_PIN, (uint8_t)BME_SCL_PIN);
                                                 
    // Création de l'objet HX711_Reader
    sensor_ptrs[sensor_index] = new HX711_Reader(
        (char*)"WG",   // Force le type en char*          // nom du capteur
        IS_ANALOG,      // analogique
        IS_CONNECTED,   // connecté
        low_power_status,// mode low power
        HX711_DT_PIN,   // DT
        HX711_SCK_PIN,  //CLK
        HX711_PWR_PIN,      // pin d'alim
        saved_offset,            // offset de calibration
        saved_scale,        // scale (kg / V)
        5               // fenêtre moyenne mobile
    );
        sensor_index++;
#endif
...
for (int i = 0; i < 4; i++) {
    if (DevAddr[i] < 0x10) Serial.print("0"); // Ajoute le zéro de tête si besoin
    Serial.print(DevAddr[i], HEX);
    if (i < 3) Serial.print(":"); // Ajoute le séparateur
}
    PRINTLN;
...
void measure_and_send(void) {
    ...
    // ch10: soil temperature, 2nd DS18B20
    // ch11: soil temperature, 3rd DS18B20
    // ch12: poids sous forme analogique
    // ch13: pression athmosphérique
    //...
    // ch20: v_bat outside tx (only for TEST_LOW_BAT)
    // ch21: current_vcc (only for TEST_LOW_BAT)
    // ch22: low voltage indication (only for TEST_LOW_BAT)
    ...
    // this is how we can wake up some sensors in advance in case they need a longer 
    //wakeup time
...
// AJOUT TK5EP
#ifdef HX711_SENSOR
    if (strncmp(sensor_ptrs[i]->get_nomenclature(), "WG", 2) == 0) {
        float weight = (float)tmp_value;
        ...
        lpp.addAnalogInput(12, weight); // poids normal
           //test
           //lpp.addAnalogInput(12, 25.5);      // Test avec une valeur fixe
    }
#endif
...
// --- SECTION TEMPÉRATURE AIR ---
#if defined DHT22_AM2305_TEMP_SENSOR || defined SHT_TEMP_SENSOR || defined BME_TEMP_SENSOR
    // On ne cherche plus que "AT" puisque tous vos capteurs (SHT ou BME) utilisent ce nom désormais
    if (strncmp(sensor_ptrs[i]->get_nomenclature(), "AT", 2) == 0) {
        // Canal 7 pour toute température d'air
        lpp.addTemperature(7, tmp_value);
    }
#endif

// --- SECTION HUMIDITÉ AIR ---
#if defined DHT22_AM2305_HUM_SENSOR || defined SHT_HUM_SENSOR || defined BME_HUM_SENSOR
    if (strncmp(sensor_ptrs[i]->get_nomenclature(), "AH", 2) == 0) {
        // Canal 8 pour toute humidité d'air
        lpp.addRelativeHumidity(8, tmp_value);
    }
#endif

// --- SECTION PRESSION (Spécifique au BME) ---
#ifdef BME_PRESS_SENSOR
    if (strncmp(sensor_ptrs[i]->get_nomenclature(), "AP", 2) == 0) {
        // Canal 13 pour la pression barométrique
        lpp.addBarometricPressure(13, tmp_value);
    }
#endif
...
#if defined USE_XLPP
// rajout tk5ep
// On déclare la variable qui est définie dans local_lorawan.cpp
    extern uint16_t Frame_Counter_Up; 
    
    // On lui injecte la valeur de ton EEPROM
    Frame_Counter_Up = my_nodeConfig.seq;
    //
...
// we use raw format for LoRaWAN
    ...
    do {
        ...
        // ADDED TK5EP
        long wait_time = (long)(endSend + rxw * DELAY_BEFORE_RCVW) - (long)millis();
        PRINT_CSTSTR("Wait for ");
        if (wait_time < 0) {
            PRINT_VALUE("%ld", wait_time); // Affiche le retard avec un signe "-"
            PRINT_CSTSTR(" (RETARD !)");
        } else {
            PRINT_VALUE("%ld", wait_time);
        }
        PRINTLN;
        //
        ...
        RXPacketL = LT.receive(message, sizeof(message), 1000, WAIT_RX);
    ...
    if (RXPacketL) {
    // ADDEDTK5EP
    // Si on arrive ici, c'est qu'un paquet a été capté !
    int16_t rssi = LT.readPacketRSSI();
    int8_t snr = LT.readPacketSNR();
    
    Serial.print("--- PAQUET REÇU ---");
    Serial.print(" RSSI local: "); Serial.print(rssi);
    Serial.print(" | SNR local: "); Serial.println(snr);
    // END
        int i = 0;
        long cmdValue;
...
// TEST
PRINTLN;
PRINT_CSTSTR("Number of sensors=");
PRINT_VALUE("%d", number_of_sensors);
PRINTLN;
...
#else
        // CONFIGURATION_RETENTION=RETAIN_DATA_RAM on SX128XnextTransmissionTimedle
