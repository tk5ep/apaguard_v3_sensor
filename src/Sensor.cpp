...
Sensor::Sensor(const char* nomenclature, bool is_analog, bool is_connected, bool is_low_power, int pin_read, int pin_power, int pin_trigger){
  ...
  if (is_low_power) {
    set_is_power_on_when_active(true);
    set_is_power_off_when_inactive(true);
  }
  else {
    set_is_power_on_when_active(false);
    set_is_power_off_when_inactive(false);   
  }
...
bool Sensor::get_is_power_on_when_active(){
  return _is_power_on_when_active;
}

bool Sensor::get_is_power_off_when_inactive(){
  return _is_power_off_when_inactive;
}
...
void Sensor::set_is_power_on_when_active(bool b){
  _is_power_on_when_active = b;
}

void Sensor::set_is_power_off_when_inactive(bool b){
  _is_power_off_when_inactive = b;
} 
